        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center lg:items-start">
          ...
          >
            ...
            <div className="flex items-center gap-4 text-sm font-medium">
              ...
              <div>
                <p className="text-white text-sm md:text-base">Direct WhatsApp Integration</p>
                <p className="text-gray-500 text-sm md:text-base">Fastest way to reach me</p>
              </div>
            </div>
          </motion.div>
          ...
          >
            <form onSubmit={handleSubmit} className="glass-card p-8 md:p-10 rounded-3xl relative overflow-hidden bg-black/40 backdrop-blur-xl border border-white/5 shadow-[0_0_40px_rgba(56,189,248,0.05)]">
              {isSuccess && (
                <motion.div 
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                ...
                >
                  <motion.div
                    ...
                    className="w-20 h-20 rounded-full bg-sky-500/20 flex items-center justify-center mb-4 border border-sky-500/30 shadow-[0_0_30px_rgba(56,189,248,0.2)]"
                  >
                    <CheckCircle2 className="w-10 h-10 text-sky-400" />
                  </motion.div>
                  <h3 className="text-3xl font-bold text-white mb-2 tracking-tight capitalize">Ready to Roll!</h3>
                  <p className="text-sky-300/80 text-sm md:text-base">Opening WhatsApp securely...</p>
                </motion.div>
              )}
              ...
              <div className="space-y-8 relative z-10">
                {/* Name */}
                <div>
                  <label htmlFor="name" className="block text-xs font-semibold capitalize tracking-widest text-sky-400/80 mb-3 ml-1">Your Name</label>
                  <input 
                    id="name"
                    value={formData.name}
                    onChange={(e) => {
                    ...
                    type="text" 
                    className={`w-full bg-white/5 border ${nameError ? 'border-red-500/50' : 'border-white/10'} rounded-xl px-5 py-4 text-white focus:outline-none focus:border-sky-500/50 focus:bg-white/10 transition-all placeholder:text-gray-600 focus:shadow-[0_0_15px_rgba(56,189,248,0.15)]`} 
                ...
                {/* Edit Options */}
                <div>
                  <label className="block text-xs font-semibold capitalize tracking-widest text-sky-400/80 mb-3 ml-1">What do you want to edit?</label>
                      ...
                      return (
                        <button
                          ...
                          className={`px-4 py-2.5 rounded-full text-sm font-medium transition-all duration-300 border ${
                            isSelected 
                              ? 'bg-sky-500/20 border-sky-500/50 text-sky-100 shadow-[0_0_15px_rgba(56,189,248,0.2)]'
                ...
                {/* Message Box */}
                <div>
                  <label htmlFor="message" className="block text-xs font-semibold capitalize tracking-widest text-sky-400/80 mb-3 ml-1">Project Details</label>
                  <textarea 
                    id="message"
                    value={formData.message}
                    onChange={(e) => setFormData({...formData, message: e.target.value})}
                    rows={4}
                    className="w-full bg-white/5 border border-white/10 rounded-xl px-5 py-4 text-white focus:outline-none focus:border-sky-500/50 focus:bg-white/10 transition-all resize-none placeholder:text-gray-600 focus:shadow-[0_0_15px_rgba(56,189,248,0.15)]" 
                ...
                <button 
                  ...
                  className="w-full bg-gradient-to-r from-sky-500 to-cyan-600 text-white font-bold py-4 rounded-xl hover:opacity-90 hover:shadow-[0_0_25px_rgba(56,189,248,0.4)] transition-all flex items-center justify-center gap-3 group disabled:opacity-70 disabled:cursor-not-allowed border border-sky-400/50 relative overflow-hidden capitalize"
                >
