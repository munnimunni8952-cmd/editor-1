    ...
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          >
            {/* Top Glow Accent */}
            <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[60%] h-[1px] bg-gradient-to-r from-transparent via-[#38bdf8]/50 to-transparent" />
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-6">
              ...
              <div>
                <label className="mb-3 block text-xs font-semibold capitalize tracking-widest text-sky-400">
                    ...
                    return (
                      <button
                        ...
                        className={`group flex items-center gap-2 rounded-full border px-4 py-2.5 text-sm font-medium transition-all duration-300 ${
                          isSelected
                            ? 'border-sky-500 bg-sky-500/10 text-white shadow-[0_0_10px_rgba(56,189,248,0.2)] scale-[1.02]'
                            : 'border-white/10 bg-black/40 text-gray-400 hover:border-white/20 hover:bg-white/5 hover:text-white'
                        }`}
                      >
                        <div className={`flex h-4 w-4 items-center justify-center rounded-full border border-white/20 transition-colors ${isSelected ? 'bg-sky-500 border-sky-500' : 'bg-transparent'}`}>
              ...
              <div>
                <label htmlFor="message" className="mb-2 block text-xs font-semibold capitalize tracking-widest text-sky-400">
                  Message (Optional)
                </label>
                <textarea
                  id="message"
                  ...
                  className="w-full rounded-xl border border-white/10 bg-black/40 px-4 py-3.5 text-white placeholder-gray-600 outline-none transition-all focus:border-sky-500/50 focus:bg-[#0B1225] focus:shadow-[0_0_10px_rgba(56,189,248,0.2)] resize-none"
              ...
              <button
                type="submit"
                className="group mt-2 flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-sky-400 to-cyan-500 px-4 py-4 text-[15px] font-bold text-black transition-all hover:scale-[1.02] hover:shadow-[0_0_15px_rgba(56,189,248,0.3)] active:scale-[0.98] capitalize"
              >
                Discuss Project
