...
export default function WorkModal() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.name || !formData.mobile || !formData.software) return;
    ...
    setIsOpen(false);
    setFormData({ name: '', mobile: '', software: '' });
  };
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className="relative w-full max-w-md overflow-hidden rounded-2xl border border-white/10 bg-[#0c0b0f]/90 p-8 shadow-2xl backdrop-blur-xl"
          >
            <button
              onClick={() => setIsOpen(false)}
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              <button
                type="submit"
                className="mt-4 flex w-full items-center justify-center rounded-xl bg-gradient-to-r from-[#4DA3FF] to-[#6EE7FF] px-4 py-3.5 text-sm font-bold text-black transition-all hover:scale-[1.02] hover:shadow-[0_0_20px_rgba(77,163,255,0.4)] active:scale-[0.98]"
              >
                Send Message
              </button>
            </form>
