...
function VideoPreviewItem({ 
  ...
  useEffect(() => {
    if (shouldLoad && videoRef.current) {
      videoRef.current.play().catch(() => {
        // Auto-play might be blocked by browser policies if not muted, but this is handled by the muted prop
      });
    }
  }, [shouldLoad]);
    ...
    >
      ...
      <div className="absolute inset-0 transition-opacity duration-300 pointer-events-none z-20">
        ...
        <div className="absolute bottom-4 right-4 md:bottom-6 md:right-6">
          <div className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-black/40 flex items-center justify-center border border-white/20 hover:border-blue-400/50 hover:bg-blue-600/30 shadow-[0_0_15px_rgba(59,130,246,0.2)] transform transition-all duration-300 backdrop-blur-md pointer-events-auto cursor-pointer"
               onClick={(e) => {
                 e.stopPropagation();
                 toggleMute();
               }}>
